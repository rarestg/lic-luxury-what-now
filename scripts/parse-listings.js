...
function parseListing(content, filename, sourceUrlById) {
  ...
  const imageUrls = [...new Set(
    (content.match(/https:\/\/images\.realty\.mx\/[^\s\)"\]]+\.jpg/g) || [])
  )].sort();

  // Extract features (between "Building & Unit Features" and next section)
  const featuresMatch = content.match(/### Building & Unit Features\n([\s\S]*?)(?=###|\nContact Agent|\nParking:)/);
  ...
  const descMatch = content.match(/Contact Agent\s*\n+([\s\S]*?)(?=\nParking:|\n### Listing Agent)/i);
  ...
  const contentUrlMatch = content.match(/https:\/\/www\.luxuryapartmentslic\.com\/[a-z0-9\-\/]+\/\d+/i);
  const contentUrl = contentUrlMatch ? contentUrlMatch[0].trim() : null;
  const url = sourceUrlById[id] || contentUrl || `https://www.luxuryapartmentslic.com/index.cfm?page=properties&id=${id}`;
...
function main() {
  ...
  const files = fs.readdirSync(LISTINGS_DIR).filter(f => f.endsWith('.md')).sort();
  ...
  console.log(`Stats: ${withSqft} with sqft, ${withImages} with images, ${sharedImages} shared image URLs`);
}
