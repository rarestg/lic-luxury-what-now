...
Usage:
  node scripts/prepare-worker-public.js            # includes static data files (dev/offline)
  node scripts/prepare-worker-public.js --no-data  # skips data files (production, UI uses /api/bootstrap)
*/
...
function main() {
  ...
  const fileCount = UI_FILES.length + (noData ? 0 : DATA_FILES.length);
  console.log(`Prepared ${PUBLIC_DIR}`);
  console.log(`Copied ${fileCount} files${copiedLocalConfig ? ' + config.local.js' : ' + empty config.local.js'}.`);
}
